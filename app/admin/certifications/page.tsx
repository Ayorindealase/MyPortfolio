...
export default function CertificationsPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/certifications').then((res) => res.json()).then(setItems);
  }, []);
  ...
  const startEdit = (item: Certification) => {
    setEditing(item);
    setCreating(false);
    setForm({ title: item.title, certId: item.certId || '', sortOrder: item.sortOrder, visible: item.visible });
  };
    ...
    <div>
      ...
      {showForm && (
        <div className="bg-[#111118] border border-[#1e1e2e] rounded p-6 mb-6">
          <h2 className="text-sm font-bold text-white mb-4">{editing ? 'Edit Certification' : 'New Certification'}</h2>
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <label className="block text-xs text-gray-400 mb-1 uppercase tracking-wider">Cert ID</label>
              <input
                value={form.certId || ''}
        ...
        {items.map((item) => (
          <div key={item.id} className="bg-[#111118] border border-[#1e1e2e] rounded p-4 flex items-center justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              {item.certId && <div className="text-xs text-gray-400 font-mono mt-0.5">ID: {item.certId}</div>}
            </div>
