...
export default function EducationPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/education').then((res) => res.json()).then(setItems);
  }, []);
  ...
  const startEdit = (item: Education) => {
    setEditing(item);
    setCreating(false);
    setForm({ degree: item.degree, field: item.field, institution: item.institution, location: item.location, gpa: item.gpa, period: item.period, sortOrder: item.sortOrder, visible: item.visible });
  };
      ...
      {showForm && (
        <div className="bg-[#111118] border border-[#1e1e2e] rounded p-6 mb-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            {(['degree', 'field', 'institution', 'location', 'gpa', 'period'] as const).map((f) => (
              <div key={f}>
                <label className="block text-xs text-gray-400 mb-1 uppercase tracking-wider">{f}</label>
                <input
                  value={form[f]}
        ...
        {items.map((item) => (
          <div key={item.id} className="bg-[#111118] border border-[#1e1e2e] rounded p-4 flex items-center justify-between gap-4">
            <div className="flex-1 min-w-0">
              <div className="font-semibold text-white text-sm">{item.degree} — {item.field}</div>
              <div className="text-xs text-gray-400">{item.institution} · {item.period}</div>
              <div className="text-xs text-gray-500 font-mono mt-0.5">GPA: {item.gpa}</div>
            </div>
